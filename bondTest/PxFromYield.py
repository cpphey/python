...
def initialize_output_dataframe():
    ...
    return pd.DataFrame(columns=[
        ...
        'delta_2', 'delta_adj_price', 'spread_to_curve', 'spread_to_worst', 'clean_price'
    ])
...
def process_bond_data(row, output_data):
    ...
    input_ytm = row['input_ytm'] /100 if pd.notna(row['input_ytm']) else None
    ...
    ytm = input_ytm
    #dirty_price = bond.dirtyPrice(desired_ytm, day_count, ql.Compounded, ql.Semiannual)
    clean_price = bond.dirtyPrice(ytm, day_counter, ql.Compounded, ql.Semiannual)
    print(f"dirtyPrice: {clean_price :.8f}%")
    ...
    if shock_size is not None:
        ...
        dirty_price_adj = ql.BondFunctions.cleanPrice(bond, yield_rate_shocked)
        dirty_price_unAdj = ql.BondFunctions.cleanPrice(bond, yield_rate)
        delta = (  dirty_price_adj - dirty_price_unAdj ) / dirty_price_unAdj
        print(f"Delta (with dummy shock of {shock_size * 100:.2f}%): {delta:.8f}")
        delta_2 = duration_modified * clean_price * (1/shock_size)
        print(f"Delta (with dummy shock of {shock_size * 100:.2f}%): {delta_2:.8f}")
        delta_adj_price = dirty_price_adj
        print(f"delta_adj_price {shock_size * 100:.2f}%): {delta_adj_price:.8f}")
    ...
    new_row = pd.DataFrame([{
        ...
        'delta_2': delta_2 if delta_2 is not None else None,
        'delta_adj_price' : delta_adj_price if delta_adj_price is not None else None,
